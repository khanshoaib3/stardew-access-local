...
- Machine detection is now primarily handled by vanilla method, introduced in SV v1.6. As a fallback, a custom data asset, , is added which other mods can edit to add untracked machines (if any).
- (Breaking) Replaced  map property method with a custom data asset, .
...
- Similarly, added APIs to programmatically add a tile to accessible tiles instead of using :
    - **AddTile** - 2 overloads
- Added APIs register handlers for detecting dynamic tiles i.e., mods can register C# delegates to conditionally check and return for a tile's content:
    - **AddTileLocationHandler** - registers a handler constrained to a specific
    - **AddTileLocationNameHandler** - registers a handler constrained to a spcecifc
    - **AddTileEventIdHandler** - registers a handler constrained to a specific
    - **AddTileHandler** - unconstrained, one per mod.
- Added APIs to register C# handlers for the mod's  and  patches, that handle speaking of elements on a (unpatched) screen.
...
- Object tracker changes:
    ...
    - Configs for these keybinds:  and
    - (Breaking) Config name for cycling between objects has been renamed from  to  and from  to  for better clarification with the addition of new configs.
- (Breaking) Use standard movement keys and left gamepad stick and d-pad for navigating the character creation menu.
    ...
    - As a replacement to large increase/decrease keys,   is added which when combined with the up and down movement keys will increase the amount in multiples of 10. Default is left shift and/or left shoulder button.
...
- Keys are suppressed for **10ms** after  starts listening, fixes the issue of left click key being registered as the new keybind as soon the button is pressed.
- Compare  element in  by reference for duplicate.
...
- Used  SMAPI API for simulating mouse clicks with primary left/right click keybinds. The secondary left/right click keybinds are left unchanged in case there's any bug since this API is new.
...
- New(en.ftl):  = [English value](https://github.com/khanshoaib3/stardew-access-local/blob/5f89bce8970140658794d2290b83160195a82bb4/stardew-access/i18n/en.ftl#L17-L20)
- New(en.ftl):  = [English value](https://github.com/khanshoaib3/stardew-access-local/blob/5f89bce8970140658794d2290b83160195a82bb4/stardew-access/i18n/en.ftl#L22-L25)
- New(en.ftl):  =
- New(menu.en.ftl):  =
- New(menu.en.ftl):  =
- New(menu.en.ftl):  =  button
- New(menu.en.ftl):  =
- New(menu.en.ftl):  =
